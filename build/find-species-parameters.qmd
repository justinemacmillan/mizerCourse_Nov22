...
We have also included some parameters that mizer ignores, namely the maximum length ever recorded `max_l`, age at maturity `mat_age`,  the maximum age `max_age` and maximum size `w_max` derived allometrically from maximum length. We find these useful as a sanity check on our choices for `w_mat`,  `k_vb` and `w_inf`. Expand the following section if you want to learn more.
