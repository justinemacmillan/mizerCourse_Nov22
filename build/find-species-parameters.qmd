...
```{r}
herring_latin <- rfishbase::common_to_sci("Herring")
herring_latin
```
...
The code below is used to extract these average spawning stock biomasses from the ICES database. 
...
To download the summary data for stocks of a particular species, the user needs to provide the name of the `species` and the `year` published.
...
### Species
We are interested in the `biomass` and `landings` of 17 species over the last ten years i.e. 2012 - 2021. This information is necessary for calibrating and tuning the `mizer` model for the Celtic Sea. So we will first create a list of the species and use it in the `getSAG` function.
...
From the data we will filter

 - data for the years 2012 - 2021
 - records where the spawning stock biomass (SSB) and landings are greater than 0
 - records where stockSizeDescription is *SSB, Spawning Stock Biomass*.
...
data_sp<- summary_data|>
  ...
  # since some records of some stocks where total catch is provided but landings and discards are missing
  mutate (catches_nw = coalesce(catches, cat))|>
  group_by(fishstock)|>
  #we shall summarise the data using geometric mean
  summarise(SSB = exp(mean(log(SSB), na.rm = T)),
            total_catch = exp(mean(log(catches_nw), na.rm = T)),
            F = exp(mean(log(as.numeric(F)), na.rm = T)))|>
   #separate the stock into species and ices areas`
  separate(fishstock, into = c('species', 'ices_area'), sep = 4)
```

Next we calculate the `yield_observed` and `biomass_observed`, which is done by getting the area in km2 for the ICES statistical areas in which the stock was assessed. 
```{r}
#load area (km2) of the ICES area/divisions
ices_areas <- read.csv("ices_area.csv")

data_ssb<- left_join(data_sp, ices_areas, by = ('ices_area' = 'ices_area'))|>
  mutate (yield_observed = total_catch/area,
          biomass_observed = SSB/area)|> filter(!is.na(biomass_observed))

```

We then add a dataframe with species codes, which we can ensure the species name is the same.
...
There is also no SSB for three species, poor cod, Norway pout and common dab. We can fill in the gaps by reviewing literature and making some assumptions. 

For example we assumed that the area assessed for blue whiting was similar to that of mackerel. We also note that the `biomass_bserved` for *boc* i.e. boarfish is smaller than `yield_observed`. This is worrying because it implies that there is not enough fish to sustain the fishery. Consequently we need to look for other SSB perhaps in the literature. One source could be from [O’Donnell et al. 2012.] (http://hdl.handle.net/10793/787) where SSB is stated as 432,882 tonnes based on an area coverage of over 306,942.11 km².

```{r}
# this code can be added to update the biomass_observed for boarfish
# data_sp<- data_sp|>mutate(biomass_observed = ifelse(species == "Boarfish",  432882/306942.11, biomass_observed))

saveRDS(data_ssb, "celtic_sea_ssb_13sp.rds")
```


We then load the results and transfer the biomass estimates into a `biomass_observed` column in the species parameter data frame.
...
3\) It is extremely important to document the origin of your parameter values and to preserve the code for extracting the values from the data sources.
