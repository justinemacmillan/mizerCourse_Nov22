...
## Exercise
Do you think you can improve on the parameter choices we have made? Do you think a species should be added or removed? If so, simply modify the corresponding entries in the `sp` data frame. Perhaps add a comment about where your values come from.

When you are happy with the species parameters, save them:
```{r}
