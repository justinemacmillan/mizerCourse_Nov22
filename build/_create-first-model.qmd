...
Of course this has now messed up our biomasses again:

```{r warning=FALSE}
plotBiomassVsSpecies(cel_model5)
```

However the discrepancies are now much smaller than they were before. So perhaps we should simply iterate the procedure to get the discrepancies even smaller. However before we do that, there is another things we need to attend to in the next step.
...
And it has also messed up our growth rates:
...
Of course there are still things wrong with this model. We will improve the model in the next two tutorials.
